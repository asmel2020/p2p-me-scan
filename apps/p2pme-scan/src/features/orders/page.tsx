...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
      <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: 'var(--color-muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        {label}
      </span>
      {children}
    </div>
...
export function OrdersPage() {
  ...
  const goToPage = useCallback((newCursor: string | undefined) => {
    navigate({ to: '/', search: { cursor: newCursor, limit }, replace: false });
  }, [navigate, limit]);
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['orders-page', cursor, limit, statusFilter, typeFilter, currencyFilter],
    queryFn: async () => {
  ...
  const totalUsdc = stats?.byCurrency.reduce((sum, c) => sum + c.totalUsdc, 0) ?? 0;
  ...
  const handleLimitChange = (value: string | null) => {
    ...
    cursorHistory.current = [];
    navigate({ to: '/', search: { cursor: undefined, limit: n }, replace: false });
  };
  ...
  return (
    <RootLayout>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: '0 0 0.25rem' }}>Orders</h1>
      ...
      <div style={{
        ...
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: 'var(--color-muted-foreground)', marginBottom: '0.25rem' }}>Total Orders</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{stats?.orders.toLocaleString() ?? '—'}</div>
        </div>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: 'var(--color-muted-foreground)', marginBottom: '0.25rem' }}>Total Events</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{stats?.events.toLocaleString() ?? '—'}</div>
        </div>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: 'var(--color-muted-foreground)', marginBottom: '0.25rem' }}>USDC Volume</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{totalUsdc ? `${totalUsdc.toLocaleString()} USDC` : '—'}</div>
        </div>
        <div className="stat-card">
          <div style={{ fontSize: '0.75rem', color: 'var(--color-muted-foreground)', marginBottom: '0.25rem' }}>Active Orders</div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.8125rem', color: 'var(--color-muted-foreground)' }}>
          {!isFetching && orders.length > 0 && (
            <span>Page {currentPage} · {orders.length} orders</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            ...
            style={{
              ...
              color: hasPrev ? 'var(--color-foreground)' : 'var(--color-muted-foreground)',
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid var(--color-border)',
              borderRadius: 'var(--radius-lg)',
              color: hasNext ? 'var(--color-foreground)' : 'var(--color-muted-foreground)',
              fontWeight: 500, fontSize: '0.8125rem',
